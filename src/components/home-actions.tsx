...
export function HomeActions({ rooms }: { rooms: Room[] }) {
  ...
  async function createGroup(event: React.FormEvent) {
    ...
    if (response.ok && result.room) setGroupList((current) => [...current, result.room]);
    setBusy(false);
  }
  ...
  async function deleteGroup(id: string) {
    ...
    const result = await response.json();
    setMessage(response.ok ? "Group deleted." : result.error ?? "Could not delete group.");
    if (response.ok) setGroupList((current) => current.filter((room) => room.id !== id));
    setBusy(false);
  }
  ...
  return <div className="mt-10 grid gap-4 lg:grid-cols-[1.3fr_.7fr]">
    ...
    <div className="glass-panel rounded-3xl p-5"><p className="eyebrow">Build a room</p><h2 className="mt-1 text-2xl">Create a group</h2><p className="font-ui mt-3 text-sm leading-6 text-[var(--muted)]">Start a focused space for a subject, hostel, event, or department.</p><form onSubmit={createGroup}><input required maxLength={80} value={groupName} onChange={(event) => setGroupName(event.target.value)} placeholder="Group name" className="font-ui mt-5 w-full rounded-xl border border-[var(--line)] bg-transparent px-3 py-3 outline-none focus:border-[var(--teal)]"/><select value={visibility} onChange={(event) => setVisibility(event.target.value as "public" | "private")} className="font-ui mt-3 w-full rounded-xl border border-[var(--line)] bg-transparent px-3 py-3"><option value="public">Public group</option><option value="private">Private group</option></select><button disabled={busy} className="auth-submit mt-3 w-full rounded-xl border border-[var(--teal)] px-4 py-3 font-bold text-[var(--teal)]"><Plus size={15} className="mr-2 inline"/> Create group</button></form><div className="mt-5 space-y-2 border-t border-[var(--line)] pt-4">{groupList.filter((room) => room.owner_id).map((room) => <div key={room.id} className="font-ui flex items-center justify-between gap-2 text-sm"><span className="truncate">{room.name}</span><button type="button" disabled={busy} onClick={() => deleteGroup(room.id)} className="text-xs text-[var(--coral)] hover:underline">Delete</button></div>)}</div></div>
